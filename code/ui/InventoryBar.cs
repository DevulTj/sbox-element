...
namespace Element.UI
{
	...
	public class InventoryBar : Panel
	{
		...
		List<Weapon.BaseWeapon> Weapons = new();

		public bool IsOpen;
		Weapon.BaseWeapon SelectedWeapon;
		...
		public override void Tick()
		{
			...
			Weapons.Clear();
			Weapons.AddRange(
				player.Children.Select( x => x as Weapon.BaseWeapon ).Where( x => x.IsValid() && x.IsUsable() ) );
		...
		public void ProcessClientInput( InputBuilder input )
		{
			...
			if ( IsOpen != wantOpen )
			{
				SelectedWeapon = Local.Pawn.ActiveChild as Weapon.BaseWeapon;
				IsOpen = true;
			}
		...
		int SlotPressInput( InputBuilder input, int SelectedIndex )
		{
			...
			var firstOfColumn = Weapons.Where( x => x.Bucket == columninput ).OrderBy( x => x.BucketWeight )
				.FirstOrDefault();
		...
		int NextInBucket()
		{
			...
			Weapon.BaseWeapon first = null;
			Weapon.BaseWeapon prev = null;
